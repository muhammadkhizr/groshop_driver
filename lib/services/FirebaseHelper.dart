// ignore_for_file: close_sinks, unused_local_variable, non_constant_identifier_names, duplicate_ignore
...
import 'package:gromartdriver/model/BlockUserModel.dart';
import 'package:gromartdriver/model/ChannelParticipation.dart';
...
class FireStoreUtils {
  ...
  static Future<OrderModel?> getOrder(String uid) async {
    DocumentSnapshot<Map<String, dynamic>> orderDocument = await firestore.collection(ORDERS).doc(uid).get();
    if (orderDocument.data() != null && orderDocument.exists) {
      // print('milaa');

      return OrderModel.fromJson(orderDocument.data()!);
    } else {
      return null;
    }
  }
  temp() async {
    DocumentSnapshot<Map<String, dynamic>> userDocument = await firestore.collection(ORDERS).doc("myDKUZjJfzadiDJA3uM4wduE8T93").get();
    OrderModel model = OrderModel.fromJson(userDocument.data()!);
    await firestore.collection(USERS).doc("myDKUZjJfzadiDJA3uM4wduE8T93").update({"orderRequestData": userDocument.data()!});
    // if(MyAppState.currentUser == null)return;
    // MyAppState.currentUser!.orderRequestData = model;
    // updateCurrentUser(MyAppState.currentUser!);
  }
  ...
  static Future updateWalletAmount({required String userId, required amount}) async {
    ...
    await firestore.collection(USERS).doc(userId).get().then((value) async {
      ...
      if (userDocument.data() != null && userDocument.exists) {
        try {
          walletAmount = userDocument.data()!['wallet_amount'] + amount;
          print('\x1b[92m Total wallet amount : ${walletAmount}');
          await firestore
              .collection(USERS)
              .doc(userId)
              .update({"wallet_amount": walletAmount}).then((value) => print("north"));
  ...
  static Future<User?> updateCurrentUser(User user) async {
    // log("MK: updating user, ${user.userID} , ${user.walletAmount}");
  ...
  static Future<String> uploadCarImageToFireStorage(File image, String userID) async {
    Reference upload = storage.child('gromart/drivers/carImages/$userID.png');
  ...
  static Future<dynamic> loginWithEmailAndPassword(String email, String password) async {
    try {
      ...
      if (documentSnapshot.exists) {
        print('\x1b[92m snapshot === ${documentSnapshot.data()}');
  ...
  static Future<dynamic> firebaseSubmitPhoneNumberCode(String verificationID, String code, String phoneNumber,
    ...
    if (user != null && user.role == USER_ROLE_DRIVER) {
      ...
      if (errorMessage == null) {
        ...
        return 'couldntCreateNewUserWithPhoneNumber'.tr;
      }
    }
  }

  static firebaseUpdateUserData(String userId, String emailAddress, File? image, File? carImage, String carName, String carPlate,
      String firstName, String lastName, String mobile, BuildContext context) async {
    try {
      ...
      if (user != null) {
        ...
        if (image != null) {
          ...
          profilePicUrl = await uploadUserImageToFireStorage(image, userId ?? '');
        }
        if (carImage != null) {
          updateProgress('Uploading car image, Please wait...'.tr());
          carPicUrl = await uploadCarImageToFireStorage(carImage, userId ?? '');
        }
        //user.active = true;
  ...
  static firebaseSignUpWithEmailAndPassword(String emailAddress, String password, File? image, File? carImage, String carName, String carPlate,
