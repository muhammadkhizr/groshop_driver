...
import 'package:gromartdriver/model/ChatModel.dart';
import 'package:gromartdriver/model/ChatVideoContainer.dart';
import 'package:gromartdriver/model/ConversationModel.dart';
import 'package:gromartdriver/model/HomeConversationModel.dart';
import 'package:gromartdriver/model/MessageData.dart';
...
import 'package:gromartdriver/ui/chat/PlayerWidget.dart';
import 'package:gromartdriver/ui/fullScreenImageViewer/FullScreenImageViewer.dart';
import 'package:gromartdriver/ui/fullScreenVideoViewer/FullScreenVideoViewer.dart';
...
class _ChatScreenState extends State<ChatScreen> {
                        ...
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          if (snapshot.hasData && (snapshot.data?.message.isEmpty ?? true)) {
                            return Padding(
                              ...
                              child: Center(child: showEmptyState('noMessagesYet'.tr(), 'sendANewMessageAndItWillShowUpHere'.tr())),
                            );
                          } else {
  ...
  Widget buildSubTitle(User friend) {
    String text = friend.isActive ? 'activeNow'.tr() : 'lastSeenOn'.tr(args: ['${setLastSeen(friend.lastOnlineTimestamp.seconds)}']);
            ...
            children: <Widget>[
              ...
              Positioned.directional(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                        color:
                            homeConversationModel.members.firstWhere((element) => element.userID == messageData.senderID).isActive ? Colors.green : Colors.grey,
  ...
  Future<bool> _checkChannelNullability(ConversationModel? conversationModel) async {
    if (conversationModel != null) {
      ...
      ConversationModel conversation = ConversationModel(
          creatorId: user.userID, id: channelID, lastMessageDate: Timestamp.now(), lastMessage: 'sentAMessage'.tr(args: ['${user.fullName()}']));
        ...
        CupertinoActionSheetAction(
          ...
          onPressed: () async {
            ...
            if (isSuccessful) {
              ...
              _showAlertDialog(context, 'block'.tr(), 'couldntBlock'.tr(args: ['${homeConversationModel.members.first.fullName()}']));
            }
          },
        ),
        CupertinoActionSheetAction(
          ...
          onPressed: () async {
            ...
            bool isSuccessful = await _fireStoreUtils.blockUser(homeConversationModel.members.first, 'report'.tr());
            hideProgress();
            if (isSuccessful) {
              Navigator.pop(context);
              _showAlertDialog(context, 'report'.tr(), 'hasBeenBlockedAndReported'.tr(args: ['${homeConversationModel.members.first.fullName()}']));
            } else {
              _showAlertDialog(context, 'report'.tr(), 'couldntReportOrBlock'.tr(args: ['${homeConversationModel.members.first.fullName()}']));
            }
          },
        ),
      ],
